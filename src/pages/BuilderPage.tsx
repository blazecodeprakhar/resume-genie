...
export default function BuilderPage() {
  ...
  const addSkill = (type: 'technical' | 'soft') => {
    ...
    if (input.trim()) {
      // Split by commas, semicolons, newlines, periods followed by space, or periods separating words (e.g. word.Word)
      const rawParts = input.split(/[,;\n\r]|\.(?=\s|$)|(?<=[a-z]{2})\.(?=[A-Z])/);
      const newSkills: string[] = [];
      
      rawParts.forEach(part => {
        let cleaned = part.trim();
        // Remove leading bullets, hyphens, pluses, asterisks, etc.
        cleaned = cleaned.replace(/^[•\*\-\+◦▪\s]+/, '');
        // Remove leading "and " or "or "
        cleaned = cleaned.replace(/^(and|or)\s+/i, '');
        cleaned = cleaned.trim();
        
        if (cleaned && !data[key].includes(cleaned) && !newSkills.includes(cleaned)) {
          newSkills.push(cleaned);
        }
      });

      if (newSkills.length > 0) {
        store.updateData({ [key]: [...data[key], ...newSkills] });
      }
      setter('');
    }
  };
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.technicalSkills.map((s, i) => (
                      <span key={i} className="inline-flex items-start justify-between gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-1.5 text-sm font-medium text-primary max-w-full">
                        <span className="break-words flex-1">{s}</span>
                        <button onClick={() => removeSkill('technical', i)} className="hover:text-destructive shrink-0 mt-0.5"><X className="h-3.5 w-3.5" /></button>
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {data.softSkills.map((s, i) => (
                      <span key={i} className="inline-flex items-start justify-between gap-2 rounded-lg border border-border bg-muted/40 px-3 py-1.5 text-sm font-medium text-foreground max-w-full">
                        <span className="break-words flex-1">{s}</span>
                        <button onClick={() => removeSkill('soft', i)} className="hover:text-destructive shrink-0 mt-0.5"><X className="h-3.5 w-3.5" /></button>
                      </span>
                    ))}
                  </div>
                </div>
